...
class Main(tk.Frame): # группировака frame, и др изменяются
    def __init__(self, root):
        super().__init__(root)
        self.init_main() # это чтобы он выбрался
        self.db = db
        self.view_records()
    ...
    def update_record(self, name, tel, email, salary):
        ...
        WHERE ID=?""", (name, tel, email, salary,
                        self.tree.set(self.tree.selection()[0], "#1")))# метод возвращает строки
        self.db.conn.commit()
        self.view_records()
    ...
    def init_main(self):
        ...
        toolbar.pack(side=tk.TOP, fill=tk.X) # 1) Сайт закрепляет на верху 2) А филл авторастгяивание по x
        ...
        # image - иконка кнопки
        self.add_img = tk.PhotoImage(file='img/user.png')
        ...
        # кнопка обновления
        self.refresh_img = tk.PhotoImage(file='img/update.png')
...
class Child(tk.Toplevel):# если есть 1 большое окно и унего есть методы, то точно так же будет тут
    ...
    def init_child(self):  
        ...
        self.grab_set() # метод для перехвата событие приложение (отслеживает доч. классу)
        self.focus_set() # метод когда появилось дочернее окно можно было нажимать только на дочернее окно
        # дочернее окно
        ...
        self.btn_ok.bind('<Button-1>', lambda event: self.view.records( self.entry_name.get(),
                                                                       ...
                                                                       self.entry_tel.get(),
                                                                       self.entry_salary.get())) #создаем событие
...
class DB():
    ...
    def insert_data(self, name, tel, email, salary):
        self.c.execute("""INSERT OR IGNORE INTO db (name, tel, email, salary)
